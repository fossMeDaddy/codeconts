...
if [[ -n $VERSION ]]; then
    ...
    pkg_version="$(curl --silent https://raw.githubusercontent.com/fossMeDaddy/codeconts/main/version)"
fi
...
curl -# -f -L -o $pkg_bin https://github.com/fossMeDaddy/codeconts/releases/download/$pkg_version/$target_bin_name
curl_exit_code=$?
